...
from utils.schemas import green_trips_schema, taxi_zone_schema, yellow_trips_schema
...
def clean_yellow(spark):
    df = (
        spark.read.schema(yellow_trips_schema)
        .parquet(*sources["yellow_taxi"].values())
        .drop_duplicates()
...
def clean_green(spark):
    df = (
        spark.read.schema(green_trips_schema)
        .parquet(*sources["green_taxi"].values())
        .drop_duplicates()
        .filter(col("trip_distance") > 0)
        .filter(col("total_amount") >= 0)
        .filter(col("fare_amount") >= 0)
...
def clean_zones(spark):
    df = (
        spark.read.schema(taxi_zone_schema)
        .parquet(sources["taxi_zone_lookup"])
...
def transform_bronze_to_silver(force: bool = False) -> None:
    ...
    # check that silver output is exist, so doesn't need to rerun spark jobs again"
    if not force and all(
        silver_write_complete(p) for p in [silver_yellow_path, silver_green_path, silver_zones_path]
    ):
        print("Silver output already exist and completed, skip process. Pass force=True to re-run.")
        return
    ...
    yellow.write.mode("overwrite").partitionBy("pickup_date").parquet(
        f"{home_path}/silver/yellow_trips/"
    )
    green.write.mode("overwrite").partitionBy("pickup_date").parquet(
        f"{home_path}/silver/green_trips/"
    )
